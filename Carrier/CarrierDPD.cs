...
{
	public class CarrierDPD : Carrier
	{
		public ConfigCarrierDPD config;
		public bool orderSent = false;

		// ========== Config Class ==========
		public class ConfigCarrierDPD : ConfigBase<ConfigCarrierDPD>
		{
			public string inputPath = Program.ExecutableFolder + Path.DirectorySeparatorChar + "DPD" + Path.DirectorySeparatorChar + "Input";
			public string inputFilename = "Input_{ID}.csv";
			public string outputPath = Program.ExecutableFolder + Path.DirectorySeparatorChar + "DPD" + Path.DirectorySeparatorChar + "Output";
			...
			public static ConfigCarrierDPD LoadFile() {
				return Load ("config" + Path.DirectorySeparatorChar + "carrierDPD.json");
			}

			public void SaveFile() {
				Save ("config" + Path.DirectorySeparatorChar + "carrierDPD.json");
			}
		}
		...
		public CarrierDPD ()
		{
			...
			// Config:
			this.config = ConfigCarrierDPD.LoadFile ();
		...
		public override void OpenSettingsWindow () {
			CarrierDPDWindow settingsWindow = new CarrierDPDWindow (this);
			settingsWindow.Show ();
		}
		...
		public override void SendToCarrier(Order order) {
			...
			// Send:
			Program.Log (this.Name, "Sending input to DPD...");
			...
			// Wait For Responce:
			Program.Log (this.Name, "Waiting for DPD output...");
			...
			if (outputDir.GetFiles ().Length > 0) {
				...
				// Error:
						...
						Program.LogError (this.Name, "DPD has not returned a tracking number, it returned this:");
				...
				// Invalid:
					...
					Program.LogError (this.Name, "Invalid output file received from DPD.");
					order.Error = true;
					return;
				}
			}
			...
			// Error File:
				...
				Program.LogError (this.Name, "Error received from DPD, the following values were not accepted:");
			...
			// No Output:
				...
				Program.LogError (this.Name, "No output file received from DPD.");
				order.Error = true;
				return;
			}
